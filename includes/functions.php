...
function getEnhancedAnalytics($date = null, $userId = null) {
    ...
    try {
        ...
        // File attachment statistics - check if table exists
        $stmt = $pdo->prepare("SHOW TABLES LIKE 'task_attachments'");
        $stmt->execute();
        $hasAttachments = $stmt->fetch();
        ...
        // Work output statistics - check if table exists
        $stmt = $pdo->prepare("SHOW TABLES LIKE 'task_work_outputs'");
        $stmt->execute();
        $hasOutputs = $stmt->fetch();
